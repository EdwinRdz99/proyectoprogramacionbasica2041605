...
void Alta()
{
	if (n == 0)//Variable que sirve para identificar si ya existen los registros (0=no existen)
	{
		...
		{
			bool numeroRepetido = true;//Bool almacena si el verdadero o falso
			while (numeroRepetido)//Mientras el numero se repita
			{
				printf("Ingrese el Numero del item: ");
				scanf_s("%d", &nITEM[i]); // Leer el numero de articulo
				cin.ignore();

				// Validar si el número de artículo ya existe en los registros anteriores
				numeroRepetido = false;
				for (int j = 0; j < i; j++)
				{
					if (nITEM[j] == nITEM[i])//compara el registro actual con los anteriores
					{
						printf("El numero de articulo ingresado ya existe. Vuelva a intentar.\n");
						numeroRepetido = true;
						break;
					}
				}
...
void Lista()
{
	if (n != 0) // mientras existan registros
	{
		string filtro;
		int opcion;
		printf("Filtro de busqueda:\n");
		printf("1. Sin filtro\n");
		printf("2. Por consola\n");
		printf("3. Por clasificacion\n");
		printf("4. Por genero\n");
		...
		case 2:
			...
			for (int i = 0; i < numRegistro; i++)
			{
				if (strcmp(consola[i].c_str(), filtro.c_str()) == 0)//strcmp compara dos cadenas de caracteres en este caso el que agregamos lo compara con los existentes
				{
					printf("--> Registro Numero: %d\n", i + 1);
					printf("Item Numero: %d\n", nITEM[i]);
					printf("%s\n", Nom[i].c_str());
		...
		case 3:
			...
			for (int i = 0; i < numRegistro; i++)
			{
				if (strcmp(clas[i].c_str(), filtro.c_str()) == 0)//strcmp compara dos cadenas de caracteres en este caso el que agregamos lo compara con los existentes
				{
					printf("--> Registro Numero: %d\n", i + 1);
					printf("Item Numero: %d\n", nITEM[i]);
					printf("%s\n", Nom[i].c_str());
		...
		case 4:
			...
			for (int i = 0; i < numRegistro; i++)
			{
				if (strcmp(gen[i].c_str(), filtro.c_str()) == 0)//strcmp compara dos cadenas de caracteres en este caso el que agregamos lo compara con los existentes
				{
					printf("--> Registro Numero: %d\n", i + 1);
					printf("Item Numero: %d\n", nITEM[i]);
					printf("%s\n", Nom[i].c_str());
